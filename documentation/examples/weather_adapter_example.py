...
def main():
    ...
    print(
        f"  Avg precipitation: {sum(weather_data['precipitation']) / len(weather_data['precipitation']):.1f} mm/day"
    )
    print(
        f"  Avg temp (max): {sum(weather_data['temperature_max']) / len(weather_data['temperature_max']):.1f} °C"
    )
