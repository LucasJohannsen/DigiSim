...
def test_retry_dispatcher_logs_retry_attempt(tmp_path, mock_event, mock_context):
    with structlog.testing.capture_logs() as logs:
        mock_client = Mock()
        mock_client.send_event.side_effect = [httpx.TimeoutException("timeout"), None]

        from tenacity import wait_none

        dispatcher = RetryDispatcher(
            mock_client, max_attempts=3, queue_dir=str(tmp_path), _wait_strategy=wait_none()
        )
...
def test_digizert_client_logs_event_dispatched(mock_event, mock_context):
    with structlog.testing.capture_logs() as logs, patch("httpx.post") as mock_post:
        mock_response = Mock()
