...
def _make_context() -> SimContext:
    return SimContext(
        field_id=990001, field_name="Test Field", field_size=20.0,
        soil_type="sandy_loam", start_date=datetime.datetime(2026, 1, 1),
        crop_type="Potato", variety="Belana", fuel_variation=0.1,
    )


def _make_planting_plan(protections, days_to_target=0, grow_duration=90):
    return PlantingPlan(
        crop_type="Potato", variety="Belana",
        planting_period_months=(4, 5), harvest_period_months=(9, 10),
        grow_duration=grow_duration, phases=[],
        protection_plans=[ProtectionPlan(
            name="Test", description="Test",
            days_to_target=days_to_target, protections=protections,
        )],
    )
...
class TestProtectionPlanServiceAnchor:
    def test_anchor_is_planting_date(self):
        ...
        with patch("services.protection_plan_service.sim_helper.get_protection_categories", return_value=[{"id": 27, "category": "F", "name": "F"}]):
...
class TestProtectionPlanServicePruning:
    ...
    def test_sikkation_too_close_to_harvest_is_pruned(self):
        ...
        plan = _make_planting_plan(prots)
        with patch("services.protection_plan_service.sim_helper.get_protection_categories", return_value=[{"id": SIKKATION_CATEGORY, "category": "H", "name": "H"}]):
            svc = ProtectionPlanService(_make_context(), pd, plan, hd)
        names = [op.application_name for op in svc.operations]
    ...
    def test_no_pruning_when_harvest_date_none(self):
        pd = datetime.datetime(2026, 5, 11)
        prots = [Protection(day=0, type=27, name="E", amount="1.0"), Protection(day=200, type=27, name="L", amount="1.0")]
        plan = _make_planting_plan(prots)
        with patch("services.protection_plan_service.sim_helper.get_protection_categories", return_value=[{"id": 27, "category": "F", "name": "F"}]):
...
class TestProtectionOperationsPrunedEvent:
    def test_event_emitted_with_correct_counts(self):
        ...
        plan = _make_planting_plan(prots)
        bus = DomainEventBus()
        with patch("services.protection_plan_service.sim_helper.get_protection_categories", return_value=[{"id": 27, "category": "F", "name": "F"}, {"id": SIKKATION_CATEGORY, "category": "H", "name": "H"}]):
    ...
    def test_no_event_when_pruned_count_zero(self):
        pd = datetime.datetime(2026, 5, 11)
        hd = pd + datetime.timedelta(days=90)
        prots = [Protection(day=0, type=27, name="K1", amount="1.0"), Protection(day=50, type=27, name="K2", amount="1.0")]
        plan = _make_planting_plan(prots)
        bus = DomainEventBus()
        with patch("services.protection_plan_service.sim_helper.get_protection_categories", return_value=[{"id": 27, "category": "F", "name": "F"}]):
            ProtectionPlanService(_make_context(), pd, plan, hd, bus)
    ...
    def test_no_event_when_bus_none(self):
        pd = datetime.datetime(2026, 5, 11)
        hd = pd + datetime.timedelta(days=90)
        prots = [Protection(day=0, type=27, name="K", amount="1.0"), Protection(day=100, type=27, name="P", amount="1.0")]
        plan = _make_planting_plan(prots)
        with patch("services.protection_plan_service.sim_helper.get_protection_categories", return_value=[{"id": 27, "category": "F", "name": "F"}]):
...
class TestCalendarDrivenRunnerHarvestDate:
    def test_compute_harvest_date(self):
        ...
        expected = runner.planting_plan_service.planned_planting_date + datetime.timedelta(days=runner.planting_plan_service.planting_plan.grow_duration)
    ...
    def test_initialize_services_uses_planting_date(self):
        ...
        assert runner.protection_plan_service.start_date == runner.planting_plan_service.planned_planting_date
...
def _pd(v):
    return v if isinstance(v, datetime.datetime) else datetime.datetime.strptime(v, "%Y-%m-%d %H:%M:%S")
...
class TestFastForwardIntegration:
    ...
    def test_pruned_emitted_once(self, ff_sim):
        pruned = [e for e in ff_sim["domain_events"] if e.event_type == "ProtectionOperationsPruned"]
        assert len(pruned) == 1
    ...
    def test_last_sikkation_14d_before_roden(self, ff_sim):
        ...
        sikk = [_pd(e.start_date) for e in events if int(getattr(e, "worktype", 0)) == 14 and int(getattr(e, "application_category", 0)) == SIKKATION_CATEGORY]
