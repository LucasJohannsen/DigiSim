...
def test_min_moisture_level():
    ...
    assert 365 <= len(moisture_data) <= 366, (
        "Expected moisture data for 365/366 days, but got a different length."
    )
...
def test_extreme_moisture_level():
    ...
    assert irrigation.sum() > 1000, (
        "Expected irrigation to be greater than 1000 mm, but got a different value."
    )
    assert 365 <= len(irrigation) <= 366, (
        "Expected irrigation array to have 365/366 days, but got a different length."
    )
