...
class TestDomainEventIntegration(unittest.TestCase):
    ...
    def test_operation_events_sequence(self):
        ...
        if len(operation_considered) > 0:
            self.assertGreater(
                len(operation_approved), 0, "If operations are considered, some should be approved"
            )

        # If operations were approved, they should be applied
        if len(operation_approved) > 0:
            self.assertGreater(
                len(operation_applied), 0, "If operations are approved, they should be applied"
            )
    ...
    def test_crop_cycle_started_event(self):
        ...
        if len(crop_cycle_events) > 0:
            self.assertEqual(
                len(crop_cycle_events), 1, "CropCycleStarted should only be emitted once"
            )
...
class TestDecisionManagerEvents(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.decision_manager = DecisionManager(
            strategy=WorkTypePriorityStrategy(), event_bus=self.event_bus
        )
    ...
    def test_operation_considered_events(self):
        ...
        self.decision_manager.decide(ops, field_id="1", date=datetime(2025, 4, 1))
