...
from scheduler.calendar_driven_runner import (
...
class FastForwardRunner:
    ...
    def run(self) -> list[FieldOperationEvent]:
        ...
        for day in range(self.n_days):
            try:
                ...
                self.error_count += 1
                logger.error(
                    "Error during fast-forward tick", day=day, date=current_date, error=str(e)
                )
    ...
    def _export_to_json(self) -> None:
        ...
        if self.output_path:
            ...
            # Default export path
            export_dir = os.path.join(os.path.dirname(__file__), "../export/fast_forward")
        ...
        logger.info(
            "Fast-forward results exported to JSON", filepath=filepath, events=len(self.all_events)
        )
