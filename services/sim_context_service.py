...
def collect(context: SimContext, defaults=None):
    ...
    context.field_id = get_input(
        f"Enter field ID [{defaults['field_id']}]: ", int, default=defaults["field_id"]
    )
    context.field_name = get_input(
        f"Enter field name [{defaults['field_name']}]: ", str, default=defaults["field_name"]
    )
    ...
    context.start_date = get_input(
        ...
        if isinstance(defaults["start_date"], datetime.datetime)
        else datetime.datetime.strptime(defaults["start_date"], "%Y-%m-%d"),
    )
    context.crop_type = get_input(
        f"Enter crop type [{defaults['crop_type']}]: ", str, default=defaults["crop_type"]
    )
    context.variety = get_input(
        f"Enter crop variety [{defaults['variety']}]: ", str, default=defaults["variety"]
    )
    return context
