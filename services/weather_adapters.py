...
class OpenMeteoWeatherAdapter(WeatherDataAdapter):
    ...
    def __init__(self, latitude: float, longitude: float, enabled: bool = True, timeout: int = 10):
        """
    ...
    def _parse_response(self, data: dict) -> dict[str, Any]:
        ...
        dates = [datetime.datetime.fromisoformat(d).date() for d in daily.get("time", [])]
    ...
    def get_fallback_data(
        ...
        logger.info(
            "Using fallback weather data", start_date=start_date, end_date=end_date, days=len(dates)
        )
