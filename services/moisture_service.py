...
_SM_YEAR_RE = re.compile(r"grids_germany_daily_soil_moisture_\w+_(\d{4})_.*\.nc$")
...
class MoistureDataService:
    ...
    def __init__(self, context: SimContext, **kwargs):
        ...
        self.min_moisture_level = kwargs.get("min_moisture_level", MIN_MOISTURE_LEVEL)

    def get_moisture_file(self, year, depth_range) -> str:

        # download if not in current folder, else return path
        filepath = (
            f"grids_germany_daily_soil_moisture_{MOISTURE_SOIL_TYPE}_{year}_{depth_range}_v1.nc"
        )
    ...
    def gauss_to_wgs84(self, x, y):
        ...
        """
        point_gdf = gpd.GeoDataFrame(geometry=gpd.points_from_xy([x], [y]), crs="EPSG:31467")
    ...
    def find_random_coordinate_with_date(self, nc_file):
        ...
        while attempts < 10:
            ...
            return {"x": x_val, "y": y_val, "moisture_data": moisture_data}
        raise ValueError("Could not find a valid coordinate with non-blank moisture data.")
    ...
    def find_coordinate_at_coords(self, nc_file, lat, lon):
        ...
        point_gdf = gpd.GeoDataFrame(geometry=gpd.points_from_xy([lon], [lat]), crs="EPSG:4326")
    ...
    def get_moisture_data(self, **kwargs):
        ...
        try:
            nc_file_path = self.get_moisture_file(year=year, depth_range=depth_range)
            ...
            print(
                f"Warning: Moisture data for year {year} not available. "
        ...
        if field_coords is not None:
            ...
            moisture_object = self.find_coordinate_at_coords(nc_file, lat, lon)
        else:
            moisture_object = self.find_random_coordinate_with_date(nc_file)
            lat, lon = self.gauss_to_wgs84(moisture_object["x"], moisture_object["y"])

        return {
            "coords": [lat, lon],
            "dates": [
                datetime.date(year, 1, 1) + datetime.timedelta(days=i)
                for i in range(len(moisture_object["moisture_data"]))
            ],
