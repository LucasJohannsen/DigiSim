...
class FarmLoaderService:
    ...
    def validate_farm_data(self, farm_data: list[Farm]):
        ...
        for farm in farm_data:
            ...
            if len(field_ids) != len(set(field_ids)):
                ...
                duplicates = set([x for x in field_ids if field_ids.count(x) > 1])
    ...
    def _load_farms_from_api(self, api_url: str) -> list[Farm]:
        ...
        if response.status_code == 200:
            ...
            for farm in farms:
                farm["fields"] = [
                    field for field in fields_data if field.get("enterprise") == farm["id"]
                ]
    ...
    def _load_farms_from_file(self) -> list[Farm]:
        ...
        """
        with open(self.farms_file) as file:
            data = json.load(file)

        farms = []
        for farm_data in data["farms"]:
            fields = [Field(**field) for field in farm_data["fields"]]
            farm = Farm(id=farm_data["id"], name=farm_data["name"], fields=fields)
            farms.append(farm)
        return farms
